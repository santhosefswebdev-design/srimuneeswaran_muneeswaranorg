...
class Booking_cust extends BaseController
{
    ...
    public function index_old(){
      ...
      echo view('front_user/booking/index');
    ...
    public function index(){
        ...
        echo view('front_user/layout/header');
        //echo view('template/sidebar');
        echo view('front_user/booking_new/index', $data);
        //echo view('front_user/layout/footer');
    }
    ...
    public function book(){
        ...
        echo view('front_user/layout/header');
        //echo view('template/sidebar');
        echo view('front_user/booking_new/index1', $data);
        echo view('front_user/layout/footer');
    }
    ...
    public function hallbook_list(){
       ...
       // print_r($data);die;
        echo view('front_user/layout/header');
        //echo view('template/sidebar');
        echo view('front_user/booking/hallbooklist', $data);
        echo view('front_user/layout/footer');
    }
    ...
    public function add_booking(){
        ...
        echo view('front_user/layout/header');
        //echo view('template/sidebar');
        echo view('front_user/booking/add_booking', $data);
        echo view('front_user/layout/footer');
    }
    public function edit_booking(){
      ...
      echo view('front_user/layout/header');
      //echo view('template/sidebar');
      echo view('front_user/booking/edit_hallbooking', $data);
      echo view('front_user/layout/footer');
  }
    ...
    public function save_booking(){
		...
		if( !empty($data['booking_date']) && !empty($data['event_name']) && !empty($data['name']) && !empty($data['mobile_number']) && !empty($data['email']) && !empty($data['city'])) {
        ...
        if(!empty($_POST['service']))
        {
          foreach($_POST['service'] as $row){
            ...
            $this->db->table("hall_booking_service_details")->insert($packdata);
				...
				$final_amount = $data['paid_amount'];
				$paydata['hall_booking_id'] = $id;
				$paydata['date'] = $data['booking_date'];
			...
			$this->session->setFlashdata('fail', 'Please Try Again');
			$msg_data['err'] = 'Please Try Again. required field missing.';
		}
      echo json_encode($msg_data);
      exit();
    }
	public function ipay88_online_response($hall_book_id) {
		...
		if($response['status']){
			...
			$redirect_url = base_url() . '/booking_cust/print_booking/' .$hall_book_id;
	...
	public function initiate_ipay_merch_online($hall_book_id) {
		include_once FCPATH . 'app/Libraries/ipay88-master/IPay88.class.php';
		...
		$final_amt = $hall_booking['amount'];
		$MerchantCode = 'M01230';
		$MerchantKey = 'HQgUUZLVzg';
		$ref_no = 'ARCH_' . $hall_book_id;
		$refno_pay = $ref_no;
		$module = 'archanai';
		$final_amount = '1.00';
		$final_amt_str = '10000';
		...
		$ipay88->setField('ResponseURL',  base_url() . '/booking_cust/ipay88_online_response/' . $hall_book_id);
		$ipay88->setField('BackendURL',  base_url() . '/booking_cust/ipay88_online_response/' . $hall_book_id);
	...
	public function payment_process($hall_booking_id) {
		...
		if(count($hall_booking_payment_gateway_datas) > 0){
			...
			}elseif($hall_booking_payment_gateway_datas[0]['pay_method'] == 'ipay_merch_online'){
					$view_file = 'front_user/ipay88/ipay_merch_online';
					$data['hall_booking_id'] = $hall_booking_id;
					$data['submit_url'] = '/booking_cust/initiate_ipay_merch_online/' . $hall_booking_id;
					echo view($view_file, $data);
			}else{
				$redirect_url = base_url() . '/booking_cust/print_booking/' .$hall_booking_id;
				header('Location: ' . $redirect_url);
				exit;
			}
		}else{
    ...
    $final_amt = $amount * 100;
		$data = [
			...
			"reference"=> $orderid,
			'countryCode' => "MY",
			'shopperReference' => "ord_hall_".$orderid,
			'shopperEmail' => $email,
			'shopperLocale' => "en-US",
			"billingAddress" => [
				"street"=>$address,
			...
			'returnUrl' => base_url() . '/booking/print_booking/' .$orderid,
	...
	public function qrcode_generation($qr_id)
	{
		...
		if(count($kavadi_registration_check) > 0)
		{
			if(!empty($qr_id))
			{
				...
				echo view('front_user/layout/book_header');
				echo view('front_user/booking/qrcode_generation', $data);
				echo view('front_user/layout/footer');
			}
		}
		else
	...
	public function print_booking($hall_booking_id){
		...
		$view_file = 'front_user/booking/print';
   ...
   public function list_booking_print($id){
    ...
    public function print_page(){
     ...
     echo view('front_user/booking/print', $data);
    }
	public function view(){
      ...
      echo view('front_user/layout/header');
      //echo view('template/sidebar');
      echo view('front_user/booking/view_hallbooking', $data);
      echo view('front_user/layout/footer');
  }
