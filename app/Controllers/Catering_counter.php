...
class Catering_counter extends BaseController
{
	...
	public function index() {
		...
		$data['payment_mode'] = $this->db->table('payment_mode')->where("paid_through", "COUNTER")->where("catering", 1)->where('status', 1)->orderby('menu_order', "ASC")->get()->getResultArray();
		...
		$query = $this->db->query("SELECT catering FROM terms_conditions ");
		...
		echo view('frontend/catering/index', $data);
	}
			...
			if(!empty($_POST['special'])){
				foreach ($_POST['special'] as $item) {
					...
					if ($settings['setting_value'] == 1) {
						...
						if ($res_m1){
							$preparation_details = $this->db->table('madapalli_preparation_details')->where('date', $_POST['event_date'])->where('type', 3)->get()->getResultArray();
	...
	public function get_terms() {
		...
		$query = $this->db->query("SELECT catering FROM terms_conditions");
		$result = $query->getRowArray();
		$terms = json_decode($result['catering'], true);
