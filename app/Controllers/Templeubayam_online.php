...
class Templeubayam_online extends BaseController
{
	...
	public function ubayam()
	{
		...
		$data['reprintlists'] = $this->db->query("SELECT id,paid_amount,ref_no,entry_date FROM templebooking WHERE created_by = '" . $login_id . "' and booking_through = 'COUNTER' AND payment_status = 2 ORDER BY id DESC LIMIT 3")->getResultArray();
		$cur_date = date('Y-m-d');
		$ubayam_datas = $this->db->query("SELECT  tu.*, bp.name as event_name FROM  templebooking tu 
		JOIN booked_packages bp ON tu.id = bp.booking_id where ((tu.booking_through != 'DIRECT' and tu.booking_status in (1,2)) or (tu.booking_through = 'DIRECT')) and tu.booking_status != 3 and tu.booking_type = 2
		...
		// }
		$data['payment_mode'] = $this->db->table('payment_mode')->where('ubayam', 1)->where('status', 1)->where('paid_through', 'COUNTER')->orderby('menu_order', "ASC")->get()->getResultArray();
		...
		$data['deities'] = $this->db->table("deity")->where('status', 1)->select('id, name_eng, name_tamil')->get()->getResultArray();
		...
		$data['annathanam_items'] = $this->db->table('annathanam_package_items')
					->select('annathanam_items.name_eng, annathanam_items.name_tamil, annathanam_package_items.package_id')
					->join('annathanam_items', 'annathanam_package_items.item_id = annathanam_items.id')
					->where('annathanam_package_items.add_on', 0)
					->where('annathanam_items.status', 1)
					//->groupBy('annathanam_package_items.package_id')
					->get()->getResultArray();

		$annathanam_alladdon = $this->db->table('annathanam_package_items')
					->select('annathanam_items.name_eng, annathanam_items.name_tamil, annathanam_items.amount, annathanam_package_items.package_id')
					->join('annathanam_items', 'annathanam_package_items.item_id = annathanam_items.id')
					->where('annathanam_package_items.add_on', 1)
					->where('annathanam_items.status', 1)
					->groupBy('annathanam_package_items.package_id')
					->get()->getResultArray();

		$groupedAddon = [];
					foreach ($annathanam_alladdon as $item) {
						$groupedAddon[$item['package_id']][] = $item;
					}
		$data['annathanam_addon'] = $groupedAddon;
	...
	public function print_page_ubayam($id)
	{
		...
		$data['prasadam'] = $this->db->table("prasadam p")
				...
				->where('p.booking_type', 2)->where('p.booking_id', $id)->get()->getResultArray();
		...
		//$data['payment'] = $this->db->table('ubayam_pay_details')->where('ubayam_id', $id)->get()->getResultArray();
		// Replace this:
		$pay_details = $this->db->table("booked_pay_details")->where("booking_id", $id)->get()->getResultArray();
		$data['pay_details'] = $pay_details[0];

		// With this:
		$pay_details = $this->db->table("booked_pay_details")
			->where("booking_id", $id)
			->where("pay_status", 2) // Only completed payments
			->get()->getResultArray();
		...
		// Create a summary payment details array
		$data['pay_details'] = [
			'amount' => $total_paid,
			'payment_records' => $pay_details // In case you need individual records
		];
		// var_dump($data['prasadam']);
		// exit;
		echo view('frontend/templeubayam_new/print_page', $data);
	}
  ...
  public function gtpaymentdata()
	...
	public function save_repayment()
	{
		if (!empty($_POST['payment_mode']) && !empty($_POST['pay_amount']) && !empty($_POST['booking_id'])) {
			...
			if ($count > 0) {
				...
				// Calculate total paid amount
				$total_paid_query = $this->db->table("booked_pay_details")
					->selectSum('amount')
					->where('booking_id', $booking_id)
					->get()->getRowArray();
				$total_paid = $total_paid_query['amount'] ?? 0;

				if ($ubayam_details['amount'] >= ($total_paid + $pay_amount)) {
					...
					// Check if a repayment exists with the same date AND same payment mode
					$check_repayment = $this->db->table("booked_pay_details")
						->where('is_repayment', 1)
						->where('booking_id', $booking_id)
						->where('paid_date', $booking_payment_ins_data['paid_date'])
						->where('payment_mode_id', $payment_mode)
						->get()->getResultArray();

					if (count($check_repayment) != 0) {
						// Same date AND same payment mode - update existing record
						$this->db->query(
							"UPDATE booked_pay_details SET amount = amount + ? WHERE id = ? AND booking_id = ?",
							[$pay_amount, $check_repayment[0]['id'], $booking_id]
						);
						$booked_pay_id = $check_repayment[0]['id'];

						// Update payment reference number to track this addition
						$updated_ref = $check_repayment[0]['payment_ref_no'] . '-ADD' . date('His');
						$this->db->query(
							"UPDATE booked_pay_details SET payment_ref_no = ? WHERE id = ?",
							[$updated_ref, $booked_pay_id]
						);
					} else {
						// Different date OR different payment mode - create new record
						$payment_ref_no = 'REPAY-' . $ubayam_details['ref_no'] . '-' . date('YmdHis') . '-' . rand(1000, 9999);
						$booking_payment_ins_data['payment_ref_no'] = $payment_ref_no;
					...
					if ($booked_pay_id) {
						// Update the paid_amount in templebooking
					...
					echo json_encode(['status' => false, 'message' => 'Payment amount exceeds the balance amount.']);
				}
			} else {
				echo json_encode(['status' => false, 'message' => 'Invalid payment mode.']);
			}
		} else {
			echo json_encode(['status' => false, 'message' => 'Missing required information.']);
		}
		exit;
	}
	// public function save_repayment()
	...
	// 				$check_repayment = $this->db->table("booked_pay_details")->where('is_repayment',1)
	// 									->where(['booking_id'=>$booking_id])
	// 									->get()->getResultArray();
	// 				if(count($check_repayment) != 0){
	// 					$this->db->query("UPDATE booked_pay_details SET amount = amount + ? WHERE id = ? AND booking_id = ?", [$pay_amount, $check_repayment[0]['id'], $booking_id]);
	// 					$booked_pay_id = $check_repayment['0']['id'];	
	// 				} else {
	// 					$res = $this->db->table("booked_pay_details")->insert($booking_payment_ins_data);
	// 					$booked_pay_id = $this->db->insertID();
	...
	// 				$check_sum = $this->db->table("booked_pay_details")->selectSum('amount')
	// 							->where('booking_id',$booking_id)
	// 							->get()->getRowArray();
		...
		if ($booked_pay_details_cnt > 0) {
			...
			foreach ($booked_pay_details as $row) {
				...
				if (!empty($paymentmode['ledger_id'])) {
					...
					$entries['entry_code'] = 'REC' . $yr . $Mon . (sprintf("%05d", (((float) substr($qry['entry_code'], -5)) + 1)));
