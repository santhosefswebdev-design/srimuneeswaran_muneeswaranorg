...
class Prasadamsetting extends BaseController
{
	...
	public function add()
	{
        ...
        // If $id is provided, fetch the existing selections for editing
        if ($id !== null) {
            $this->db->where('user_id', $id); // Assuming user_id is used to track entries
            $data['selected_prasadam_groups'] = $this->db->table('prasadam_setting_group')->result_array();
        } else {
            $data['selected_prasadam_groups'] = [];
        }
		echo view('template/header');
		echo view('template/sidebar');
	...
	public function edit() {
		...
		$data['selected_prasadam_groups'] = $this->db->table('prasadam_setting_group')
													 ->where('prasadam_id', $id)
													 ->get()
													 ->getResultArray();
	...
	public function save(){
		...
		if(empty($id)){
			...
			// $selectedPrasadamGroups = $_POST['prasadam_group'];
			// $datas = [];
			// if ($id !== null) {
			// 	$this->db->where('user_id', $id);
			// 	$this->db->delete('prasadam_selections');
			// } else {
			// 	$id = $this->session->userdata('user_id'); 
			// }
	
			// foreach ($selectedPrasadamGroups as $prasadamId) {
			// 	$amount = $this->input->post('amount_' . $prasadamId);
			// 	$datas[] = [
			// 		'prasadam_id' => $prasadamId,
			// 		'amount' => $amount,
			// 		'user_id' => $id
			// 	];
			// }
	
			// $this->db->insert_batch('prasadam_setting_group', $datas);


			$this->db->table('prasadam_setting_group')->delete(['prasadam_id' => $id]);
				if(!empty($_POST['prasadam_group'])){
					foreach($_POST['prasadam_group'] as $arow) {
						$adata = array();
						$adata['prasadam_id'] = $insid;
						$adata['prasadam_group_id'] = $arow;
						$adata['amount'] = $_POST['amount_' . $arow];
						$builder2 = $this->db->table('prasadam_setting_group')->insert($adata);
					}
				}
