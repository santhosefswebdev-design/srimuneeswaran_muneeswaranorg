                                                ...
                                                <div class="radio-group">
                                                    <?php foreach ($prasadam_groups as $index => $group): ?>
                                                        <div class="radio-item">
                                                            <input type="radio" id="prasadam_group_<?php echo $group['id']; ?>" name="prasadam_group" value="<?php echo $group['id']; ?>" class="form-control prasadam-group-radio" <?php echo $index === 1 ? 'checked' : ''; ?>>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-5" style="text-align:center;">
                                                        <div class="form-group form-floating">
                                                            <div class="form-line">
                                                                <select id="time-slot" name="time" class="form-control">
                                                    ...
                                                    <div class="col-md-7" id="time-picker-container">
                                                        <div class="form-group form-float">
                                                            <div class="form-line">
                                                                <label for="hour" class="form-label">Select Time <span style="color: red;">*</span></label>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-4 col-xs-4" align="left" style="margin-bottom: 0;">
															...
															<label id="submit_mob" class="btn submit_btn btn-success btn-lg waves-effect">Pay</label>
                                                        </div>
...
<script>
    ...
    $("#submit_mob").click(function(){
        var totalAmount = parseFloat($("#tot_amt").val());
...
$(document).ready(function() {
    function loadProducts(groupId) {
        $.ajax({
            url: "<?php echo base_url('prasadam/fetch_prasadam_settings'); ?>",
            type: "POST",
            data: { prasadam_group_id: groupId },
            success: function(response) {
                var productsHtml = '';
                $.each(response, function(index, product) {
                    productsHtml+= `<div class="col-md-4 col-lg-3 col-sm-6 col-xs-12" style="padding-left: 0px;">
                                        <div class="prod" id="${product.id}" data-id="prod${product.id}" onclick="addtocart(${product.id})">
                                            <img src="<?php echo base_url(); ?>/uploads/prasadam_setting/${product.image}" width="200" height="80" alt="image" />
                                            <div class="detail">
                                                <h5 class="name" id="nm_${product.id}" data-id="${product.id}">${product.name_tamil}<br>${product.name_eng}</h5>
                                                <h4 id="amt_${product.id}" data-id="${product.proamt}">SGD ${parseFloat(product.proamt).toFixed(2)}</h4>
                                                <input type="hidden">
                                            </div>
                                        </div>
                                    </div>`;        
    ...
    // Load products on page load for the default selected group
    var initialGroupId = $('input[name="prasadam_group"]:checked').val();
    loadProducts(initialGroupId);

    // Load products on radio button change
