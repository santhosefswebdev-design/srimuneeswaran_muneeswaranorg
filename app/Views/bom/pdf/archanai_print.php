...
<tr>
<td colspan="2">
<h3 style="text-align:center;">BOM ARCHANAI REPORT </h3>
<h3 style="text-align:center;"> [From : <?= date('d-m-Y', strtotime($pdfdata['fdate'])); ?> To <?= date('d-m-Y', strtotime($pdfdata['tdate']));?>] </h3>
    <table class="tab" border="1" width="100%" align="center">
        ...
        <tbody>
			...
			{
                $archanai_data = $db->table('archanai')->where('id',$row['dedection_id'])->get()->getRowArray();
				$archanai_name = $archanai_data["name_eng"]." / ".$archanai_data["name_tamil"];
				$archanai_raw_mat_data = $db->table('archanai_raw_material_items')
											->where('product_id',$row['dedection_id'])
											->where('raw_id',$row['item_id'])
											->get()->getRowArray();
				$archanai_count = 	(float)$row['quantity'] / (float)$archanai_raw_mat_data['qty'];					
