                            ...
                            <tbody style="height:auto; margin-bottom:30px;">
                                ...
                                <tr>
                                  ...
                                  <td style="width: 10%;padding: 5px 0px!important;text-align:center;">
                                    <a class='btn btn-primary' style='font-size: 13px;font-weight: bold;padding: 8px 10px;background: #2196F3;border: 1px solid #2196F3;' title='Print' href='<?php echo base_url(); ?>/archanai_booking_cust/reprint_booking/<?php echo $reprintlist['id']; ?>' target='_blank'>Print</a>
                                  </td>
                                </tr>
                                <?php
                                  $ire++;
...
function displayCart() {
  ...
  {
    output += "tr><td colspan='4' align='center'><img src='https://rajamariamman.grasp.com.my/assets/archanai/images/cart_is_empty.png' class='img-fluid' style='width:150px; margin:0 auto;'></td></tr>";
  }
  else
  {
...
$("#rasi_id").change(function(){
    ...
    if(rasi != "")
    {
      //console.log(rasi_id);
      $.ajax({
        url: '<?php echo base_url();?>/archanai_booking_cust/get_natchathram',
        type: 'post',
        data: {rasi_id:rasi},
        dataType: 'json',
          ...
          $('#natchathram_id').val(response.natchathra_id);
            ...
            if(str !="") {
                ...
                $('#natchathra_id').append('<option value="">Select Natchathiram</option>');
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>/archanai_booking_cust/get_natchathram_name',
                    type: 'post',
                    data: {id:value},
                    dataType: 'json',
                    success:function(response)
...
function save_archanai(sep_print = 0){
    ...
    ({
      type:"POST",
      url: "<?php echo base_url(); ?>/archanai_booking_cust/save?sep_print=" + sep_print,
      data: $("form").serialize(),
      beforeSend: function() {    
        $("#submit, #submit_sep").prop('disabled', true);
        $("#loader").show();
      ...
      success:function(data)
      {
        ...
        if(obj.err != ''){
			...
			window.open("<?php echo base_url(); ?>/archanai_booking_cust/payment_process/" + obj.id, "_blank", "width=680,height=500");
