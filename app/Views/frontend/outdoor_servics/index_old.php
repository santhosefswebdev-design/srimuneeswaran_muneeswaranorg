              ...
              <div id="products" class="products row scroll" style="background: #f4f5fa;">
                  <div class="col-xl-3 col-sm-6 col-lg-3 col-md-4">
                    <div class="card" style="cursor:pointer;">
                        <div class="card-body d-flex flex-column justify-content-between" id="1" data-id="prod1" onclick="addtocart(1)">
                            <img class="img-fluid prod_img" src="<?php echo base_url(); ?>/uploads/prasadam_setting/1720587009_idli%20sambar%20chutney.jpeg" alt="image" />
                            <div class="d-flex justify-content-between align-items-center mb-2 mt-2" style="flex-direction: column;">
                                <p class="mb-0 text-muted arch" id="nm_1" data-id="1">name tamil<br>name eng</p>
                                 <h4 id="amt_1" data-id="name"></h4>
                                <input type="hidden">
                            </div>
                        </div>
                        </div>
                    ...
                    <div class="card-body">
                              ...
                              <div class="modal-content">
                                  ...
                                  <div class="modal-body p-4" style="padding-bottom:10px;">
                                      <div class="text-center">
                                        ...
                                        <div class="col-md-6">
                                          <div class="form-group">  
                                            <label class="form-label" for="collection_date">Bookng Date </label>
                        ...
                        <ul class="payment">
                          ...
                          <li><input type="radio" name="pay_method" id="cb4" value="online" />
                            <label for="cb4"><i class="mdi mdi-qrcode"></i> Online</label>
                          </li>
                         ...
                         <!--<div class="pay_amt d-flex justify-content-between align-items-center" style="width:100%; margin-top:5px;">
                            ...
                            <p class="mb-0"><span class="bal-amt total-cart"></span></p>
                             ...
                             <!-- <div class="col-xl-12 col-sm-12 col-lg-12 col-md-12">
                                 <input disabled value="PRINT" class="btn btn-info submit_btn" id="submit_mob">
                             </div> -->
                              <div class="col-xl-12 col-sm-12 col-lg-12 col-md-12">
...
<script>
  ...
  function addtocart(ids) {
    ...
    if (!$(".cart-table tr.all_close").length) {  // Check if there are no items in the cart
        $(".show-cart img").remove();  // Remove the empty cart image if present
        $(".cart-table").show();  // Show the cart table
    }

    $(".cart-table .all_close").each(function() {
        if ($(this).data('id') == ids) {
            exist = true;
            var qtyInput = $(this).find('.row_qty');
            var newQty = parseInt(qtyInput.val()) + 1;
            qtyInput.val(newQty).trigger('keyup');  // Trigger man_qun to update totals
        }
    });

    if (!exist) {
        var count = $('.cart-table .all_close').length; // Count existing items to index properly
        var text1 = `<tr class="all_close" data-id="${ids}" id="remov${ids}">
            <td style="width: 40%;"><input type="hidden" id="id_${ids}" name="prasadam[${count}][id]" value="${ids}"><p>${text}</p></td>
            <td style="width: 20%;"><input type="text" style="text-align: center;" class="row_tot" readonly name="tot" id="tot_${ids}" value="${amt}"></td>
            <td style="width: 12%;"><button class="btn btn-info" style="font-size:10px;" onclick="remove(${ids})" id="remove">X</button></td>
        </tr><tr class="all_close" data-id="${ids}" id="remov${ids}"><td colspan="3"><b>Free Services : </b></td></tr>`;
  ...
  $(document).ready(function() {
    $('input[name="prasadam_group"]').change(function() {
        validateUserDetails();  
    });
    ...
    $("#ar_add_btn").click(function(event) {
        ...
        if (!validateUserDetails()) {
          ...
          $errorMsg.text("Please fill in this details.").css('color', 'red');

          setTimeout(function() {
            $errorMsg.text("");  // Remove the text from the error message
            resetBorders();      // Reset borders for all fields that were checked
          }, 2000);

        } else {
            console.log("User details are valid. Proceeding with submission.");
            // Here you might want to close the modal if everything is okay
            // $("#myModal").modal('hide');
            // save_archanai();  // Or call save function if this is appropriate here
        }
    });
    ...
    function validateUserDetails() {
      ...
      var conditionallyRequiredFields = groupId === '1' ? ['#collection_date', '#s_time'] : [];
      ...
      fieldsToCheck.forEach(function(field) {
          ...
          if ($input.val().trim() === '') {
              ...
              $input.after('<div class="error_msg">This field is required.</div>'); 
              allValid = false;
          } else {
              $input.css('border', ''); 
              $input.next('.error_msg').remove(); 
