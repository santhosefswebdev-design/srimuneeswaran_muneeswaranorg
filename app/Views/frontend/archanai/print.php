...
<body style="max-width: 80mm;max-height: 355px;font-weight: 600;font-family: monospace;">
	<p>
		<?php if (!empty($temp_details['ar_image'])) { ?>
			<img src="<?php echo base_url(); ?>/uploads/main/<?php echo $temp_details['ar_image']; ?>" style="width:120px;"
				align="center">
		<?php } else { ?>
	...
	foreach ($booking as $row) { ?>
		<p style="text-align: left;">
			...
			<?= $row['name_tamil']; ?><br>&nbsp;&nbsp;
			<?php 
			// Check if 'description' is an array and display each item
			if (is_array($row['description'])) {
				foreach ($row['description'] as $desc) {
					echo $desc . '<br>&nbsp;&nbsp;';
				}
			} else {
				echo $row['description'] . '<br>&nbsp;&nbsp;'; // Display single description
			}
			?>
	...
	<br>
	<br>
	<?php /*<img src="<?php echo $qrcdoee; ?>" style="display:block;margin:0 auto;" width="150" height="150">
