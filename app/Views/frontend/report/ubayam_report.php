...
<style>
    ...
    .table {
        ...
        table-layout: fixed;
        /* Ensures the table adjusts based on content */
    }
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="row clearfix">
            ...
            <th style="width:8%;">Ref No</th>  <!-- ADDED -->
            <th style="width:9%;">Event Name</th>
            <th style="width:15%;">Name</th>
            <th style="width:12%;">Mobile Number</th>
            <th style="width:6%;">Amount(S$)</th>
            <th style="width:9%;">Paid Amount(S$)</th>
            <th style="width:9%;">Balance Amount(S$)</th>
            <th style="width:10%;">Remarks</th>  <!-- ADDED -->
            <th style="width:6%;">Status</th>
            <th style="width:8%;">Action</th>
            <th style="width:12%;">Admin Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            </tbody>
                                    ...
                                    <div align="right" id="totals_table_container" style="margin-top:20px;">
                                        <table id="totals_table" border="0" style="width:300px;">
                                            <tbody></tbody>
                                        </table>
                                    </div>
                                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="editBookingModalLabel"><i class="fa fa-edit" style="vertical-align:middle;"></i> Edit Booking</h4>
            ...
            <div class="modal-body">
                ...
                <div class="form-group" style="margin-top:15px;">
                    ...
                    <span class="date-error" id="date_error_msg"><i class="fa fa-warning" style="font-size:14px;vertical-align:middle;"></i> This date is already booked. Please select another date.</span>
                </div>
                <div class="form-group" style="margin-top:15px;">
                    <label class="form-label-custom">Remarks</label>
                    <textarea id="edit_remarks" class="form-control" rows="3" placeholder="Enter Remarks"></textarea>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="btn_save_booking"><i class="fa fa-save" style="vertical-align:middle;"></i> Save Changes</button>
            </div>
        </div>
    </div>
</div>
...
<script>
        ...
        report = $('#datatables').DataTable({
            ...
            ajax: {
                ...
                dataSrc: function(json) {
                    // Build totals table here
                    if(json.totals && json.totals.length > 0) {
                    var totals = json.totals[0];
                    var rows = '';
                    rows += '<tr><td style="text-align: center;"><h6><b>Total Amount</b></h6></td><td><h6>:' + totals.total_amount.toFixed(2) + '</h6></td></tr>';
                    rows += '<tr><td style="text-align: center;"><h6><b>Paid Amount</b></h6></td><td><h6>:' + totals.paid_amount.toFixed(2) + '</h6></td></tr>';
                    rows += '<tr><td style="text-align: center;"><h6><b>Balance Amount</b></h6></td><td><h6>:' + totals.balance_amount.toFixed(2) + '</h6></td></tr>';
                    $('#totals_table tbody').html(rows);
                    }
                    return json.data;
                }
            }
    ...
    $(document).on('click', '.btn-edit-booking', function () {
        ...
        $('#edit_loading').show();
        $.ajax({
            url: "<?php echo base_url(); ?>/report_online/get_booked_dates_online",
            type: "POST",
            data: {
                booking_id: bookingId,
    ...
    $('#btn_save_booking').on('click', function () {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>/report_online/update_booking_name_date_online",
            type: "POST",
            data: {
                booking_id: bookingId,
                name: name,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#alert-modal').modal('show', { backdrop: 'static' });
                    $("#spndeddelid").css("color", "green").text(response.message);
                    setTimeout(function() {
