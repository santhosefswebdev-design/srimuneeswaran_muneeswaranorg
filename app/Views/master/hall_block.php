...
<section class="content">
    <div class="container-fluid">
        <div class="block-header">
            <h2> HALL BOOKING <small>Booking / <b>Hall Blocking</b></small></h2>
        </div>
        <!-- Basic Examples -->
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Hall Blocking</h4>
      </div>
      <div class="modal-body">
        <form action="<?php echo base_url(); ?>/hallbooking/block_event_add" method="POST">
            ...
            <div class="body">
                ...
                <div class="row clearfix">
                    <div class="col-sm-6" id="show_hide_delete" align="left" style="background-color: white;padding-bottom: 1%;display:none;">
                        <button id="delete_hall_block" class="btn btn-danger btn-lg waves-effect">DELETE</button>
                    </div>
                    <div class="col-md-12" id="change_colmdname" align="right" style="background-color: white;padding-bottom: 1%;">
                        <button type="submit" name="submit_hallblocking" class="btn btn-success btn-lg waves-effect">SUBMIT</button>
                    </div>
                </div>                    
            </div>
            </div>
        ...
        var doubleClick = false;
         $('#calendar').fullCalendar({
            ...
            events: function(start, end, timezone, callback) {
                $.ajax({
                    url: "<?php echo base_url();?>/hallbooking/block_event_list",
                    dataType: 'json',
         ...
         select: function (start, end, allDay) {
            ...
            if(date >= today){
                ...
                $.ajax({
                    url: "<?php echo base_url();?>/hallbooking/blocked_event_check",
                    type: "POST",
         ...
         eventDrop: function (event, data) {
           ...
           $('#myModal').modal("show");
           $.ajax({
                 url: "<?php echo base_url();?>/hallbooking/block_event_update",
                 data: {
                     title: event.title,
                     start: start,
                     end: end,
  ...
  $("#delete_hall_block").click(function(){
    ...
    //{
        ...
        if (eventid) {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url();?>/hallbooking/block_event_delete",
                data: {
                        id: eventid
                },
                success: function (response) {
