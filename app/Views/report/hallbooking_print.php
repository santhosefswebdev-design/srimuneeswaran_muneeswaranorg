...
<tr><td colspan="2">
<h3 style="text-align:center;"> Hall Booking <?php  
		if($sts == 1) 
			echo ' - Booked';
                else if($sts == 2)  echo ' - Completed';
                else if($sts == 3)  echo ' - Canceled';
	
	?> </h3>
<h3 style="text-align:center;"> [From : <?= date("d-m-Y", strtotime($fdate)); ?> To <?= date("d-m-Y", strtotime($tdate));?>] </h3>
    <table border="1" class="tab" width="100%" align="center">
        <thead>
            <tr>
                <th>SNO</th>
                <th>Booking Date</th>
        ...
        <tbody>
            <?php $i=1; $total = 0; $paid = 0; $balance = 0; foreach($data as $row) { 
